// ============================================================
// SkyGuard Weather App - Client-Side API Service
// Direct calls to Open-Meteo APIs from the browser/native webview.
// Mirrors the exact data transformation logic from server routes
// so the app works standalone (e.g. Capacitor APK) without a backend.
//
// Data Sources:
//   Weather:  https://api.open-meteo.com (free, no key, CORS-enabled)
//   AQI:      Open-Meteo Air Quality API (primary)
//             WAQI World Air Quality Index (fallback)
//   Geocode:  https://geocoding-api.open-meteo.com (free, no key)
// ============================================================
...
// heavily polluted locations (e.g. Delhi at 550). These are NOT errors — they
// indicate genuinely hazardous air quality. We CAP at 500 (the max EPA level)
// rather than reject them, because falling back to an unreliable WAQI demo
// token would give WORSE data (e.g. WAQI returning 76 for Delhi which is
// clearly wrong when the actual AQI is 500+).
function validateAqi(aqi: number): { valid: boolean; capped: number } {
  if (typeof aqi !== 'number' || isNaN(aqi)) {
    return { valid: false, capped: 0 };
  }
  // AQI below 0 is invalid (sensor error or missing data)
  if (aqi < 0) {
    return { valid: false, capped: 0 };
  }
  // AQI above 500 exceeds the US EPA scale but is REAL data from the model.
  // Cap at 500 ("Very Hazardous") — do NOT reject the data.
  if (aqi > 500) {
    console.log(`[AQI] Capping AQI ${Math.round(aqi)} at 500 (exceeds EPA scale but is real data)`);
...
async function fetchAqiFromWAQI(lat: number, lon: number): Promise<AirQualityData | null> {
  try {
    ...
    if (!response.ok) {
      console.warn(`[AQI] WAQI API returned status ${response.status}`);
      return null;
    }
    ...
    if (json.status === 'error') {
      console.warn('[AQI] WAQI API error:', json.data || json.message || 'unknown error');
      return null;
    }
    ...
    if (rawAqi < 0) {
      console.warn('[AQI] WAQI returned invalid AQI:', rawAqi);
      return null;
    }
...
async function fetchRealTimeAQI(lat: number, lon: number): Promise<AirQualityData> {
  ...
  // This returns modelled AQI including us_aqi which can exceed 500 for
  // heavily polluted cities. We cap at 500 but always trust the data.
  try {
    ...
    if (aqResponse.ok) {
      ...
      if (aqJson?.current && typeof aqJson.current.us_aqi === 'number') {
        ...
        if (validation.valid) {
          console.log(`[AQI] Open-Meteo AQI for ${lat},${lon}: ${rawAqi} (capped: ${validation.capped})`);
          return buildAqFromOpenMeteo(aqJson.current);
        }
        // Only fall through if truly invalid (NaN, negative)
        console.warn(`[AQI] Open-Meteo AQI ${rawAqi} is invalid, trying WAQI fallback`);
      }
    }
    console.warn('[AQI] Open-Meteo AQ API failed or returned no data, trying WAQI fallback...');
  } catch (err) {
    console.warn('[AQI] Open-Meteo AQ API error:', err instanceof Error ? err.message : err);
  }

  // Source 2: WAQI World Air Quality Index (fallback — uses demo token, may be rate-limited)
...
// ============================================================
// Public API: fetchWeatherClient
// Fetches weather + AQI in parallel (mirrors server route logic)
// ============================================================

export async function fetchWeatherClient(lat: number, lon: number): Promise<WeatherData> {
...
// ============================================================
// Public API: fetchAirQualityClient
// Fetches AQI only (for lightweight live refresh)
// ============================================================

export async function fetchAirQualityClient(lat: number, lon: number): Promise<AirQualityData> {
  return fetchRealTimeAQI(lat, lon);
}

// ============================================================
// Public API: fetchGeocodeClient
// Fetches geocoding results from Open-Meteo
// ============================================================
...
// Popular cities for default display (same as server route)
...
export async function fetchGeocodeClient(q: string): Promise<GeoResult[]> {
  ...
  try {
    const res = await fetchWithRetry(
      `https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(sanitized)}&count=8&language=en&format=json`,
      { timeoutMs: 8000 },
      2
    );
  ...
  return searchFallbackCities(sanitized);
}
